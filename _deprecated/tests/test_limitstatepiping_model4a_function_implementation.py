...
def test_calc_Z_combin_piping(
    ...
    model = limitstatepiping_model4a_function_implementation.calc_Z_combin_piping(
        dist_L_geom=i1,
        dist_BUT=i2,
        dist_BIT=i3,
        L3_geom=i4,
    ...
    assert model[0] == pytest.approx(Z_u, rel=1e-2, abs=1e-2)
    assert model[1] == pytest.approx(Z_h, rel=1e-2, abs=1e-2)
    assert model[2] == pytest.approx(Z_p, rel=1e-2, abs=1e-2)
