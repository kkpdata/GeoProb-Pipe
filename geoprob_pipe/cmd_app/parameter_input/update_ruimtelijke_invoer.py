...
def add_parameters(app_settings: ApplicationSettings):
    ...
    while True:
        ...
        try:
            ...
            new_parameters = ", ".join(parameter_list)

        except Exception:
            print(
                BColors.OKBLUE,
                f"Geen parameters toegevoegd. '{parameter_input}' is geen geldige invoer.",
                BColors.ENDC,
            )
            continue

        valid_input = all(param in valid_list for param in current_parameters)
    ...
    parameters = current_parameters + new_parameters.split(",")
...
def update_metadata_parameters(app_settings: ApplicationSettings, parameters: str):
    """
