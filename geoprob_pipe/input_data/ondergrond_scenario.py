# TODO: Op dit moment is het OndergrondScenario-object niet in gebruik door de overgang naar het nieuwe input-form.
#  Mogelijk wordt die overbodig, of in beperkte vorm opnieuw gemaakt.
...
# from geoprob_pipe.input_data.data_validation import (check_attribute_already_exists, check_attribute_in_overview)
# from geoprob_pipe.input_data.utils import (
#     generate_parameter_dict_for_variable, strip_suffix_from_list_parameter_names)
#
#
# class OndergrondScenario:
#
#     # Metadata attributes of the OndergrondScenario class. These are stored as class-level type hints to make the
#     # attributes visible to static type checkers (e.g. Pylance).
...
#
#         # Create ondergrondscenarios from df. Note that the created OndergrondScenario is linked to the corresponding
#         # Vak
...
#                 raise KeyError(f"Vak ID '{row["vak_id"]}' (corresponding to scenario '{ondergrond_scenario_id}') "
...
#                 raise ValueError(f"Duplicate ondergrondscenario_id: scenario '{ondergrond_scenario_id}' already "
#                                  f"exists in vak '{vak.id}'")
#
