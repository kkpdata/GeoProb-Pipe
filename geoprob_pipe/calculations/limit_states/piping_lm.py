"""Alternatieve implementatie van de grenstoestandsfuncties voor het
WBI-model, model 4a en MORIA. Alle grenstoestandsfuncties worden
gecombineerd in één functie per model. De fysische componenten worden
geïmporteerd vanuit de subpackage `physical_components.piping`. Deze
implementatie is bedoeld om de leesbaarheid en onderhoudbaarheid van
...
def limit_state_model4a(
    ...
    L_voorland = pc_piping.calc_lengte_voorland(L_intrede=L_intrede,
                                                L_but=L_but)
    lambda_voorland = pc_piping.calc_lambda_voorland(kD_wvp=kD_wvp,
    ...
    r_exit = pc_piping.calc_r_exit_model4a(
        kD_wvp=kD_wvp, D_wvp=D_wvp, c_voorland=c_voorland,
        c_achterland=c_achterland, L_but=L_but, L_bit=L_bit,
        L_achterland=L_achterland, L_voorland=L_voorland)
    d_deklaag = pc_piping.calc_d_deklaag(mv_exit=mv_exit, top_zand=top_zand)
    phi_exit = pc_piping.calc_phi_exit(polderpeil=polderpeil, r_exit=r_exit,
                                       buitenwaterstand=buitenwaterstand)
    dphi_c_u = pc_piping.calc_dphi_c_u(
        d_deklaag=d_deklaag, gamma_sat_deklaag=gamma_sat_deklaag,
        gamma_water=gamma_water)
    i_exit = pc_piping.calc_i_exit(phi_exit=phi_exit, h_exit=h_exit,
                                   d_deklaag=d_deklaag)
    dh_c = pc_piping.calc_dh_c(
    ...
    z_u = modelfactor_u * dphi_c_u - (phi_exit - h_exit)
    z_h = (modelfactor_h * i_c_h) - i_exit
    z_p = (modelfactor_p * modelfactor_ff * modelfactor_3d
           * modelfactor_aniso * modelfactor_ml * dh_c) - dh_red
    z_combin = max(z_u, z_h, z_p)

    return (z_u, z_h, z_p, z_combin, h_exit, r_exit, phi_exit, d_deklaag,
            dphi_c_u, i_exit, L_voorland, lambda_voorland,
...
def limit_state_moria(  # TODO: Naam moria vervangen voor iets generieks?
        # Geometry parameters
        # TODO: Moet L_but L_buk worden? Of L_spreidingslengte?
        L_intrede: float, L_but: float,
        # Boundary condition parameters
        buitenwaterstand: float, buitenwaterstand_gemiddeld: float,
        polderpeil: float, mv_exit: float,
        # Subsoil property parameters
        lambda_voorland: float, phi_exit_gemiddeld: float, r_exit: float,
        top_zand: float, k_wvp: float, D_wvp: float,
        d70: float, gamma_sat_deklaag: float,
        # Model property parameters
        modelfactor_u: float, modelfactor_h: float, modelfactor_p: float,
        modelfactor_ff: float, modelfactor_3d: float,
        modelfactor_aniso: float, modelfactor_ml: float, i_c_h: float,
        r_c_deklaag: float, d70_m: float, gamma_korrel: float, v: float,
        theta: float, eta: float,
        # Constants
        g: float, gamma_water: float,
) -> Tuple[float, ...]:
    """ Grenstoestandsfuncties volgens het WBI-model met
    grondwaterstroming conform MORIA model.
