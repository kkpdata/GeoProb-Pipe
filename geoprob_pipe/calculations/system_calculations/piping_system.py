from geoprob_pipe.calculations.system_calculations.base_system_calculation import BaseSystemCalculation
...
class PipingSystem(BaseSystemCalculation):

    def __init__(
            self,
            system_variables_setup_function: Callable,
            system_variable_distributions: List[Dict],
            system_models: List[Callable],
    ):
        """
    ...
    def assign_variables(self):
        ...
        for item in self.system_variable_distributions:
            ...
            if 'minimum' in item.keys():
                self.project.variables[name].minimum = -1
