...
class MoriaSystemBuilder(BaseSystemBuilder):
    ...
    def build_instances(self, geoprob_pipe: GeoProbPipe) -> List[PipingSystemReliabilityCalculation]:
        """ """

        # project_settings = self.construct_project_settings(df_settings=df_settings)  # TODO: Replace

        # Gather input
        df_expanded = run_expand_input_tables(geopackage_filepath=geoprob_pipe.app_settings.geopackage_filepath)
        ...
        for index, row in df_unique_combos.iterrows():
            ...
            calc = PipingMORIASystemReliabilityCalculation(
