...
class ActionImportHRDLocationsFromHydraNLDatabase(Action):
    ...
    def execute(self):
        dir_to_db = self.state.question_answer.retrieve(question="dir_hydranl_db")
    ...
    def should_run(self) -> bool:
        if not (self.state.question_answer.retrieve("import_hrd") == "Hydra-NL database" and
                self.state.question_answer.retrieve("dir_hydranl_db") is not None):
            return False
        return True
