...
def _generate_parameter_dict(attr_name_without_suffix: str, df_overview_row: pd.Series, df_row: Optional[pd.Series] = None) -> dict:
    def _get_value(suffix: str, default_col: str):
        ...
        if df_row is not None and col in df_row and not pd.isna(df_row[col]):
            ...
            # If df_row is None (i.e. a constant is passed) or the value is NaN, use the default value from df_overview (if available)
    ...
    parameter_dict = {
                ...
                "distribution": df_overview_row["parameter_distribution"],   # Note: all supported distribution strings can be found in probabilistic_library.statistic.DistributionType
    ...
    if parameter_dict["distribution"] == "deterministic":
        ...
        parameter_dict["dispersion_value"] = _get_value(parameter_dict["dispersion_type"], "parameter_default_value_spreiding")    
