...
class Toolkit:
    ...
    def use_precalculated_results(self, dstability_overview: pd.DataFrame, mapping_tkx_stix: pd.DataFrame) -> None:
        ...
        self.overview = self.overview.merge(
            mapping_tkx_stix[['stix_name', 'tkx_path']], 
            on='stix_name', 
            how='left'
        )
    ...
    def use_new_calculations(self, dstability_overview: pd.DataFrame, path_output_folder: Path, path_work_dir: Path) -> None:
        ...
        """        
        self.overview = dstability_overview.copy(deep=True)  # Use the overview of the DStability class as basis
        self.results = self.start_toolkit_calculations(path_output_folder, path_work_dir)

    def preprocessing_input_tkx(self, path_work_dir: Path) -> list[Path]:
        ...
        for stix in self.overview["stix"]:
            ...
            try:
                ...
                raise ValueError(f"The .tkx file {path_modified_tkx} is not valid anymore after preprocessing modifications!")
            
            if not tkx_model_factor_in_variables(tkx_project):
                raise ValueError(f"The .tkx file {path_modified_tkx} has no model factor defined in the 'variables' tab. Make sure you enable 'add model factor' under tab Model->Response Parameters and select the correct operation (multiply or divide)")

            if not tkx_model_factor_dist_params_correct(tkx_project):
                raise ValueError(f"The .tkx file {path_modified_tkx} has a variable 'model factor' defined in the 'variables' tab, but it appears that it has incorrect distribution parameters since it's currently deterministic with a value of 1.")
    ...
    def postprocessing_output_tkx(self) -> None:
        ...
        for tkx in self.overview["tkx"]:
            ...
            try:
                ...
                raise ValueError(f"The output .tkx file {tkx["path"]} is not valid anymore after postprocessing modifications!")

    def start_toolkit_calculations(self, path_output_folder, path_work_dir: Path) -> ToolkitResults:
        ...
        self.overview["filepath_work_dir_tkx"] = self.preprocessing_input_tkx(path_work_dir)

        # Create overview of output .tkx files that will be created and start calculations
        self.overview["tkx"] = [{"path": path_output_folder / temp_tkx.name, "name": temp_tkx.name} for temp_tkx in self.overview["filepath_work_dir_tkx"]]
        run_parallel_toolkit_calculations(self.settings.ptk_console_path, self.overview["filepath_work_dir_tkx"], self.overview["tkx"].apply(lambda tkx: tkx["path"]))

        # Postprocess output .tkx files
        self.postprocessing_output_tkx()
