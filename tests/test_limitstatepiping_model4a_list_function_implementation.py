...
from app.helper_functions import limitstatepiping_model4a_list_function_implementation
...
def test_LimitStatePipingModel4a(
    ...
    X = [i1,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15,i16,i17,i18,i19,i20]
    assert limitstatepiping_model4a_list_function_implementation.Z_u(X) == pytest.approx(Z_u, 0.0001)
    assert limitstatepiping_model4a_list_function_implementation.Z_h(X) == pytest.approx(Z_h, 0.0001)
    assert limitstatepiping_model4a_list_function_implementation.Z_p(X) == pytest.approx(Z_p, 0.0001)
    assert limitstatepiping_model4a_list_function_implementation.calc_Z_combin_piping(X)[7] == pytest.approx(expected_L_kwelweg, 0.0001)
    assert limitstatepiping_model4a_list_function_implementation.calc_Z_combin_piping(X)[9] == pytest.approx(expected_dHc_piping, 0.0001)
