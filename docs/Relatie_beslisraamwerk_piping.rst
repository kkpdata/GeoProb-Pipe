...
Het Beslissingsondersteunend raamwerk piping (BRP) :cite:t:`BRP_2024` beschrijft een aanpak om aspecten te beschouwen die niet in de rekenregels zijn meegenomen. Voor bepaling van de overstromingskans van het pipingmechanisme is het belangrijk om ook de aspecten te beschouwen die niet in de rekenregels zijn meegenomen. 

In het BRP zijn 14 factsheets beschreven die piping beïnvloeden. Ook worden handelingsperspectieven gegeven om met deze aspecten om te gaan. In deze handleiding wordt beschreven hoe deze aspecten rekenkundig kunnen worden meegenomen in de overstromingskansberekening met GeoProb-Pipe. Het kennisniveau van deze aspecten bepaalt de implementatie in GeoProb-Pipe.

1. Opbarsten: Sterkte deklaag
Het is erg aannemelijk dat de deklaag enige weerstand tegen scheuren biedt. Met de huidige 
data is de scheurbestendigheid echter nog niet te kwantificeren. Er is geen gevalideerd model beschikbaar.
Daarom is dit aspect niet geimplpementeerd in GeoProb-Pipe. Wel zou via een modelfactor opbarsten :math:`m_{u}` gebruikt kunnen worden om gevoeligheidsberekeningen te doen.
...
andere stijghoogtemodellen gebruikt kunnen worden.
...
experimenten uitgevoerd om de hypothese te bewijzen en kwantificeren. 
